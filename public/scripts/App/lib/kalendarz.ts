import {Miesiac, MiesiacInterface} from "../helper/Miesiac";

export class Kalendarz {
    private input: HTMLInputElement;
    private static dniTygodnia: Array<string>;
    static dniTygodniaAng: Array<string>;
    private static readonly ileDniWWnierszu = 42;
    private data: Date;
    ...
    public uruchom(data: Date) {
        this.data = data;

        let input: any = $("#" + this.input.id),
            rodzic: any = input.parent();
        ...
        let poprzedniMiesiac: Miesiac = new Miesiac(data, -1);

        let obecnyMiesiac: Miesiac = new Miesiac(data, 0);

        let nastepnyMiesiac: Miesiac = new Miesiac(data, 1);
        ...
        let zaczytalemPierwszyTydzien = false,
            odKtoregoIndexuZaczac: number = obecnyMiesiac.pobierzIndexStartowy(),
            dniPoprzedniegoMiesiaca: Array<number> =
                poprzedniMiesiac.pobierzTabliceIloscDniMiesiaca(odKtoregoIndexuZaczac);
         ...
         */
        for (let i = 1, j = 1, k = odKtoregoIndexuZaczac; i <= Kalendarz.ileDniWWnierszu; i++, j++) {

            let komorka = document.createElement("div");
    ...
    private stworzDziecko(klasa: string): HTMLDivElement {
        let naglowek = document.createElement("div");
        naglowek.setAttribute("class", klasa);
        return naglowek;
    }

    private stworzLegende() {
        for (let dzien in Kalendarz.dniTygodnia) {
            let dzienTygodnia = this.stworzDziecko(Kalendarz.dniTygodnia[dzien]);
            dzienTygodnia.innerHTML = Kalendarz.dniTygodnia[dzien];
            this.element['legenda'].appendChild(dzienTygodnia);
        }
    }

    public miesiacWstecz() {
        document.body.querySelector('.kalendarz').remove();
        let miesiacWstecz: Miesiac = new Miesiac(this.data, -1);
        this.uruchom(miesiacWstecz.pobierzObiektDaty());
    }

    public miesiacWPrzod() {
        document.body.querySelector('.kalendarz').remove();
        let miesiacWstecz: Miesiac = new Miesiac(this.data, 1);
        this.uruchom(miesiacWstecz.pobierzObiektDaty());
    }

    public odswiezKalendarzZZaznaczonymiDniami(dniDoZaznaczenia: Array<Date>) {
        document.body.querySelector('.kalendarz').remove();
        let miesiacWstecz: Miesiac = new Miesiac(this.data, 0);
        this.uruchom(miesiacWstecz.pobierzObiektDaty());
        this.zaznaczDni(dniDoZaznaczenia);
    }

    public zaznaczDni(dniDoZaznaczenia: Array<Date>) {
        console.log("dni do zaznaczenia", dniDoZaznaczenia);
        for(let i = 0; i <= dniDoZaznaczenia.length - 1; i++){//cos tu chyba dniDoZaznaczenia = 0 a właściwie -1
            let data: Date = dniDoZaznaczenia[i];
            let id: string = (data.toISOString().split('T'))[0];
            if(document.getElementById(id) != null){
                if(document.getElementById(id).classList.contains('wybrany_dzien')){
                    let klasy = document.getElementById(id);
    ...
    public okreslId(miesiac: Miesiac, dzien: number): string{
        let rokMiesiac: string = miesiac.pobierzRokMiesiac();
        let sformatowanyDzien: string = (dzien < 10) ? "0" + dzien : dzien.toString();
